#!/usr/bin/env bash
# ============================================================================
# 저금통(피기뱅크) 백엔드 로컬 시연 스크립트
#
# 사전조건: 다른 터미널에서 서버가 떠 있어야 합니다.
#   mvn spring-boot:run
#
# 이 스크립트는 src/main/resources/data.sql 에 적재된 시연용 데이터
# (고객 C0000000001~003, 계좌 110-2233-4455 등)를 그대로 사용합니다.
# 서버를 껐다 켜도 ./data 폴더(H2 파일 DB)가 남아있는 한 데이터가 유지되므로,
# 이 스크립트를 두 번째 실행하면 "이미 활성 저금통 보유(DUPLICATE_SIGNUP)"처럼
# 이전 실행의 결과가 남아 일부 단계가 다르게 동작할 수 있습니다.
# 처음부터 다시 보려면: 서버를 끄고 `rm -rf data/` 후 재기동하세요.
#
# 사용법: ./demo.sh   (jq가 설치되어 있으면 응답이 보기 좋게 출력됩니다)
# ============================================================================
set -uo pipefail
...
# ── 1.3 동전모으기(자동저축) 배치 ───────────────────────────────────────────
# 스냅샷은 "오늘"의 DDA 잔액을 적재하고, 배치는 "내일" 날짜로 실행해
# 스냅샷(전일자=오늘)을 사용한다. 오늘 막 가입한 저금통은 오늘 배치 대상에서
# 제외되므로(당일가입 제외 규칙), 배치 실행일을 하루 뒤로 지정해야 새 저금통이
# 대상에 포함된다.
...
echo
echo "=========================================================================="
echo " 데모 종료. 각 단계의 [HTTP 코드]와 reasonCode를 README.md의 표와 비교해 보세요."
echo "=========================================================================="
